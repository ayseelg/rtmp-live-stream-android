...
- StateFlow, ViewModel
- **Dependency Injection:** Hilt gereksinimi tamamen kaldırılarak yerine kütüphaneye özgü `StreamViewModelFactory` entegre edilmiştir (Tamamen bağımsız/DI-Agnostic mimari).
- minSdk: 29

## CI/CD ve Otomasyon Süreçleri

Projede **GitHub Actions** altyapısı kurularak sürekli entegrasyon (CI) ve sürekli dağıtım (CD) süreçleri otomatikleştirilmiştir:

### 1. Otomatik Test / Build (Android CI)
Her `main` dalına kod gönderildiğinde (`push` veya `pull_request`):
- Projedeki hatalar denetlenir (Lint).
- Uygulama otomatik olarak derlenir.
- Geliştiricinin veya tester'ın indirebilmesi için **Action -> Artifacts** kısmına indirilebilir, hazır bir `app-debug.apk` yüklenir.

### 2. Kütüphane Yayınlama (Auto-Publish Library Release)
Bu kütüphaneyi kullanan diğer projeleri manuel güncellemekten kurtarmak için otomasyon eklendi. `main` dalına yeni bir commit atıldığında:
- Action sistemi kütüphaneyi (`.aar`) olarak derler.
- `v1.0.X` formatında yeni bir GitHub Release (Sürüm) oluşturur.
- Geliştiricinin attığı **Commit mesajını** otomatik olarak o sürüme "Güncelleme Notu (Release Notes)" olarak ekler.
- Diğer geliştiriciler GitHub'da hazır ve derlenmiş paket halini hemen kendi projelerinde versiyon numarasıyla update edebilirler.
...
Kütüphane kendi içinde herhangi bir Dependency Injection arcına (örn. Hilt) ihtiyaç duymaması için **StreamViewModelFactory** yapısını barındırmaktadır. Bu yüzden uygulamanızda herhangi bir bağımlılık aracı kurmadan rahatça kullanabilirsiniz.
...
Activity veya Fragment kodunuzda ilk olarak kütüphanenin kendi Factory'si aracılığıyla ViewModel'i kurun ve bellekte takılı kalmaması için mutlaka lifecycle bağlamasını yapın:
```kotlin
// Kütüphanenin oluşturduğu Factory kullanarak agnostik çağırım
...
- **Dependency Fix:** SDK içerisindeki `OpenGlView` sınıflarının dış projelere yansıtılması için Gradle `api` yapısı oluşturuldu. Hilt bağımlılığı tamamen silinerek, dışarıdan kullanılabilmesi adına `%100 DI-Agnostic` ve saf hale getirildi. Artık dışarıdan kullanan paketler (Hilt, Koin vs. kurmak kaygısı olmadan) yalnızca `StreamViewModelFactory()` aracılığı ile yayına entegre olabilir.
